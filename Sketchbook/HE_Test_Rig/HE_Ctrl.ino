...
void asi_setSpeed_internal()
{
  ...
  else
  {
    ...
    rounded = round(y);
    if (rounded < 0) { rounded = 0; }
    if (rounded >= 4096) { rounded = 4096 - 1; }
    ModRTF_PresetParam(ArxPaxRegAddr_Remote_Throttle_Voltage, rounded);
    asi_pendingReply = 1;
    asi_lastSpeed = rounded;
  }
  asi_pendingSpeed = -1;
