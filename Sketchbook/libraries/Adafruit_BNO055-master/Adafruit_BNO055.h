...
#error "On an arduino Zero, BNO055's ADR pin must be high. Fix that, then delete this line."
...
#endif
	bool  begin					(i2c_t3 *wire, adafruit_bno055_opmode_t mode = OPERATION_MODE_NDOF);
	...
	bool writeSensorOffsetsToEEPROM(int startAddress);
	bool readSensorOffsetsFromEEPROM(int startAddress);
